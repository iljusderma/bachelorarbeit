...
# ╔═╡ 4b015d30-7be2-11ee-32a7-21b300462fcb
using Random, Statistics

# ╔═╡ 29f85a4f-abe1-4554-95da-4f1e81a6362b
using Plots

# ╔═╡ 8164a82d-56a6-4145-a256-759bfcedc553
using ProfileView

# ╔═╡ 0400b7f1-de09-4149-8402-870104028787
begin
L = 1000
α = 0.3
β = 0.7
...
# ╔═╡ a7ba9a87-a2fd-42b9-8eaa-09b78080a944
"""
    step!(state::AbstractVector)

Update the state of a 1D system according to a simple rule.

The `state` argument is an `AbstractVector` representing the current state of the system. The function updates the state of the system according to the following rule:

- With probability `α`, insert a particle at the left boundary of the system.
- With probability `β`, remove a particle from the right boundary of the system.
- If there is a particle at position `i` and an empty site at position `i+1`, move the particle to the right.

The function modifies the `state` argument in place and returns nothing.
"""
function step!(state::AbstractVector)
	L = length(state)
...
# ╔═╡ afeb22c8-c0ce-485a-989f-b8dcfee13e09
snapshots = 50

# ╔═╡ d891ea44-a859-4b02-a64c-5bc2b8b8794a
densities = let
	state = falses(L)
	state[1:Int(L/2)] .= 1
	densities = zeros(20, snapshots)
	avr_length = Int(L / 20)
	for i in 1:snapshots
		...
		for j in 1:20
			@views densities[j,i] = mean(state[avr_length*(j-1)+1:avr_length*j])
		end
	end
	densities
end

# ╔═╡ 0ee2d9f5-febf-4383-bf63-158098600d4d
let

	ρ = []
	anim = @animate for i in 1:snapshots
		@views d = densities[:,i]
		plot(range(0,1,20), d;
			lims=(0,1),
		)
	...
	gif(anim, tempname()*".gif", fps = 10)
end

# ╔═╡ e94c6ec0-fcac-4caa-a861-3b63ef3a1232
plot(mean(densities; dims=1)')
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "320a3b129c4ff3cd13fe237b73e055908680c9e9"
