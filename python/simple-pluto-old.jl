...
begin
	using Plots, Statistics, Random, LaTeXStrings, PlotThemes, ProfileView
	theme(:lime)
end
...
begin
	L = 100
	iterations = 10*1000 # comparable with time
...
function SLhop(p, index::Int64) # sub lattice hop
	L = length(state)
	# check if even or odd iteration
    if index%2 == 0
		# generate random for comp. with p
        rand_vals = rand(L)
        for i in 1:(L-1)
...
function SLUpdate(iterations)
	# generate a matrix L x iterations
	all_states = zeros(Float64, L, iterations)
	# perform updates
...
function RANDUpdate(iterations)
	# generate a matrix L x iterations
	all_states = zeros(Float64, L, iterations)
	# perform updates
	for i in 1:iterations
...
begin
	@profview begin
	all_states = RANDUpdate(iterations)
	...
	total_density = vec(mean(all_states, dims=1))
	# plot densityprofile
...
begin
	scatter(total_density, label="$rates", title="Time evolution of the density", ms=2, msw=0)
	ylabel!(L"\langle \rho \rangle")
	xlabel!("Iterations")
end

# ╔═╡ 5d6f0a1e-1719-4fe9-a248-32b1041d3822
heatmap(all_states'[1:2000, :])
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "6d89e0164bc377e97167ab4d0de7b5fd1cb63d57"
